...
function ServiceOverView() {
  ...
  const [reviewsForService, setreviewsForService] = useState(null);
  ...
  useEffect(() => {
    fetchService();
    fetchReviewsForService(6, 0);
  }, []);

  useDidUpdateEffect(() => {
    if (reviewsForService) {
      chunkArrayInGroups(reviewsForService, 3);
    }
  }, [reviewsForService]);
  ...
  const fetchService = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BE_URL}/services/${id}`,
        {
          method: "GET",
          //   headers: {
  ...
  const fetchReviewsForService = async (limit, offset) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BE_URL}/reviews/?serviceId=${id}&limit=${limit}&offset=${offset}`,
        {
          method: "GET",
          //   headers: {
          //     "Content-type": "application/json",
      ...
      if (response.ok) {
        const data = await response.json();
        console.log("FETCH DATA: ", data);
        setreviewsForService(data.reviews);
      ...
      <div className="govuk-grid-row ">
        <div className="govuk-grid-column-full">
          <h1 className=" govuk-heading-l govuk-!-margin-bottom-8 ">
            {t("ClientCenterOverView-1")} {service?.name}
          </h1>
        </div>
      </div>
      <Container fluid>
